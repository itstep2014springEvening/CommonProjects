...
enum Algorithm {fcfs, rr};

int main(int argc, char **argv)
{
    if(argc < 2 || argc > 3)
    {
        printf("Wrong usage. 2 or 3 arguments\n");
        return 1;
    }
    ...
    else if(strcmp("rr", argv[1]) == 0)
    {
        if(argc != 3)
        {
            printf("Wrong usage. Round robin must have count of quantums\n");
            return 1;
        }
        ...
        if(rrTurn <= 0)
        {
            printf("Third parameter must be positive integer, stupid!!!");
            return 2;
        }
    }
    ...
    while(countOfTerminatedProcesses != n)
    {
        ...
        if(algorithm == rr && currentQuantum % rrTurn == 0)
        {
            roundRobin(head);
        }
